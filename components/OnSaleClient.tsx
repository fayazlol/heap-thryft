...
const OnSaleClient: React.FC<OnSaleClientProps> = ({ user, salelistings }) => {
  const [listings, setListings] = useState<Listing[]>(salelistings);
  const [listingsCount, setListingsCount] = useState(salelistings.length);
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(Math.ceil(salelistings.length / ITEMS_PER_PAGE));
  ...
  const fetchListings = async () => {
    try {
      const response = await fetch('/api/filtersidebar/onsale', {
        method: "POST",
        headers: {
