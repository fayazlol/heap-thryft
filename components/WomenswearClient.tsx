...
import { Card, CardBody, CardFooter, Image, Link, Divider, Input, Button } from "@nextui-org/react";
...
interface WomenswearClientProps {
...
const WomenswearClient: React.FC<WomenswearClientProps> = ({ initialListings, user }) => {
  const [listings, setListings] = useState<Listing[]>(initialListings);
  const [listingsCount, setListingsCount] = useState(initialListings.length);
  const [currentPage, setCurrentPage] = useState(1);
  const [totalPages, setTotalPages] = useState(Math.ceil(initialListings.length / ITEMS_PER_PAGE));
  ...
  const fetchListings = async () => {
    try {
      const response = await fetch('/api/filtersidebar/womenswear', {
        method: "POST",
        headers: {
    ...
    <main className="bg-[#fafafa] min-h-screen">
      <div className="flex bg-white text-black">
        ...
        <main className="w-4/5 p-4 text-black">
          <h1 className="text-2xl font-bold mb-4">Womenswear</h1>
...
export default WomenswearClient;
